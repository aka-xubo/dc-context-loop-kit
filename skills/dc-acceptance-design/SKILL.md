...
- 新建场景分配 SCN ID 时，使用当前范围内未占用的三位序号，格式固定为 `SCN-001`、`SCN-002`、`SCN-003`；不得拼接需求名称、草稿状态、标题或其他语义。对同一场景生成草案和事件时复用同一个 SCN ID，更新场景沿用已有 ID。
...
- 业务承诺不变，参与者、条件、动作、结果或业务不变量需要调整：这是 `SPEC_CHANGE`，相对最新已确认 SPEC 生成增量并一次确认；仅在无法可靠表达时重建完整 SPEC。
- 业务承诺不变，验证责任、AST 或验证环境模式需要调整：这是 `SPEC_CHANGE`，相对最新已确认 SPEC 生成增量并一次确认；仅在无法可靠表达时重建完整 SPEC。
