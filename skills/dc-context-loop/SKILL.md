...
这是 Deep Crew 专属的长期循环技能。Issue 的全部相关评论保存协作历史；本地 Markdown、结构化 YAML、证据文件和缓存保存当前定义与执行材料。

Issue 读取由 `dc-context-loop` 统一协调，不由节点技能自行触发。完整 `dc-issue-intake` 只在读取门禁命中时触发：首次处理 Issue、设计 REQ、正式验收、用户明确要求重新读取，以及 SPEC/IMPLEMENTATION 继续所需上下文缺失、冲突、关键引用无法确认或用户明确说明 Issue 已变化。SPEC 和 IMPLEMENTATION 在上下文有效且没有刷新信号时复用当前已确认定义和 Issue context，不因节点动作本身重新读取；普通交互也复用当前动作上下文。时间和评论 ID 只作为阅读历史时的排序依据。

```text
读取门禁命中时完整 intake → 模型总结当前上下文 → 判断本轮唯一节点 → 调用至多一个节点协调技能 → 发布前确定性校验 → 按 API 结果结束本轮
```

需求、实现和验收都可以持续多轮进行。每轮只处理一个责任节点：最多调用一个节点技能，最多发布一个结构化事件。事件发布动作返回成功、失败或结果未知后，立即结束本轮；下一轮先按读取门禁判断复用或刷新上下文，不在同一轮自动切换下游节点。

## 角色定位与边界

使用 `dc-issue-intake` 获取实时 Issue Context Package；不要使用陈旧评论缓存。只有用户提供或确认唯一 Issue 标识后才能执行 intake；无法唯一确定仓库或 Issue 对应的本地上下文时，先询问，不猜测。节点技能复用总协调器提供的当前有效 context；不维护本地游标、评论水位、完整评论长期缓存或多个上下文版本。未执行完整 intake 时，不得声称已检查 Issue 最新评论或确认线上历史未变化。

本技能是 Issue 级总协调器，不只是评论读取器或评论渲染器。它负责在读取门禁命中时让模型从完整 intake 恢复当前理解，或在门禁未命中时复用当前有效上下文，判断当前应进入哪个节点，并调用至多一个对应的节点协调技能；它在本轮完成一个节点动作后停止，不在同一轮自动切换下游节点。普通追问、节点内部调用和事件发布不因动作本身自动重新 intake；需要新鲜上下文时由缺失、冲突、关键引用不可确认、Issue 已变化或用户显式刷新触发。
...
`dc-context-loop` 的核心职责是语义理解和路由，不是把评论字段机械合并成状态机。读取门禁命中时，模型以本次完整 intake 为输入；门禁未命中时，模型以当前会话中仍然有效的 Issue context 和已确认定义为输入：
...
读取门禁按可观察条件而非对话轮次计算：首次处理 Issue、设计 REQ、正式验收开始前和用户明确刷新时执行完整 intake；SPEC/IMPLEMENTATION 只有在当前上下文缺失、存在冲突、关键引用无法确认、用户明确说明 Issue 已变化时才在动作前刷新。验收失败后重新设计或调整实现时不因动作本身自动刷新，仍按同一组条件判断。普通交互、当前节点内部澄清和事件发布前校验不自动触发完整 intake。

完整刷新失败或刷新后仍无法裁决时，不调用 SPEC/IMPLEMENTATION 节点技能，也不猜测生成规格或实现计划。刷新成功后，后续路由和前置引用检查必须使用本次返回的完整 context。
...
“在线存在”至少要求：事件已由当前有效 Issue context 读取或确认，事件附件存在且可下载/解析，事件 `node`、稳定 ID、引用关系和确认状态可从机器内容复核。评论正文中的摘要、失效附件、旧事件或本地文件不能单独满足前置条件；若当前 context 无法完成这些确认，先触发完整 intake。
...
1. 只有在首次处理、设计 REQ、正式验收、用户明确刷新，或 SPEC/IMPLEMENTATION 的当前 context 缺失、冲突、关键引用无法确认、Issue 已变化时，才通过 `dc-issue-intake` 完整读取 Issue 评论；上下文有效时，SPEC/IMPLEMENTATION 复用当前已确认 REQ、SPEC 和 Issue context，不因节点动作本身刷新。不得用无效或无法定位的旧缓存、评论副本或模型记忆替代有效 context。普通交互复用当前动作上下文。
2. 若没有用户提供或确认的唯一 Issue 标识，先请求确认；确认前不得调用 Issue 读取 API。事件发布前校验不以发布动作自动触发 intake。
...
7. 处理本轮唯一节点动作；每轮最多发布一个结构化事件，事件发布动作返回成功、失败或结果未知后都结束本轮。若节点未发布事件但需要用户决策、人工门禁或外部环境，也结束本轮并明确暂停原因。下游节点在下一轮先按读取门禁判断复用或刷新上下文，再进行路由；本轮输出只报告事实、结果和下一轮入口。

总协调器的输出不是“已写评论”这一动作，而是“当前上下文、当前责任节点、节点技能执行结果、下一步路由”。评论只是跨轮次保存状态的载体。

### 1. Intake 和上下文理解

在读取门禁命中的动作开始且 Issue 标识已由用户提供或确认时，调用 `dc-issue-intake` 获取 Issue、完整评论和必要附件，并确认 intake 覆盖状态为 `FULL`。门禁未命中时沿用当前有效 Issue context；不要用无效或无法定位的旧缓存、评论副本或模型记忆替代有效 context；不要再运行独立的时间线归约脚本，也不要把结构化事件的字段合并结果当成自动状态机。模型直接阅读本次返回的完整原始时间线（若触发 intake），区分结构化交付事实、普通讨论、用户最新指令、已确认决策、冲突和未决问题，再形成当前上下文理解。普通交互沿用当前动作已获取的上下文，除非出现刷新条件。
