...
Issue 读取由 `dc-context-loop` 统一协调，不由节点技能自行触发。完整 `dc-issue-intake` 只在读取门禁规定的关键动作触发：首次处理 Issue、设计 REQ、设计 SPEC、设计实现方案、正式验收，以及用户明确要求重新读取。每个关键动作都独立读取并把当次完整原始时间线交给模型理解；普通交互复用当前动作上下文。时间和评论 ID 只作为阅读历史时的排序依据。

```text
关键动作前完整 intake → 模型总结当前上下文 → 判断本轮唯一节点 → 调用至多一个节点协调技能 → 发布前确定性校验 → 按 API 结果结束本轮
```

需求、实现和验收都可以持续多轮进行。每轮只处理一个责任节点：最多调用一个节点技能，最多发布一个结构化事件。事件发布动作返回成功、失败或结果未知后，立即结束本轮；下一个关键动作在其动作入口重新 intake，不在同一轮自动切换下游节点。

## 角色定位与边界

使用 `dc-issue-intake` 获取实时 Issue Context Package；不要使用陈旧评论缓存。只有用户提供或确认唯一 Issue 标识后才能执行 intake；无法唯一确定仓库或 Issue 对应的本地上下文时，先询问，不猜测。当前关键动作的节点技能可复用该动作已读取的 context；跨关键动作不得复用旧 context，也不得创建额外的上下文版本副本。

本技能是 Issue 级总协调器，不只是评论读取器或评论渲染器。它负责在关键动作开始前让模型从该次完整 intake 恢复当前理解，判断当前应进入哪个节点，并调用至多一个对应的节点协调技能；它在本轮完成一个节点动作后停止，不在同一轮自动切换下游节点。普通追问、节点内部调用和事件发布不因动作本身自动重新 intake；需要新鲜上下文时由下一关键动作或用户显式刷新触发。
...
`dc-context-loop` 的核心职责是语义理解和路由，不是把评论字段机械合并成状态机。每次关键动作触发完整 intake 后，模型必须以以下材料为输入：
...
读取门禁按动作而非对话轮次计算：首次处理 Issue、进入 REQ/SPEC/实现方案设计动作前、正式验收开始前以及用户明确刷新时各自独立执行完整 intake；验收失败后重新设计或调整实现时，在对应动作前重新执行。普通交互、当前节点内部澄清和事件发布前校验不自动触发完整 intake。
...
1. 只有在首次处理、进入 REQ/SPEC/实现方案设计、正式验收或用户明确刷新时，才通过 `dc-issue-intake` 完整读取 Issue 评论；不得用更早动作的 Issue context、评论缓存或模型记忆替代当前关键动作的 intake。普通交互复用当前动作上下文。
...
7. 处理本轮唯一节点动作；每轮最多发布一个结构化事件，事件发布动作返回成功、失败或结果未知后都结束本轮。若节点未发布事件但需要用户决策、人工门禁或外部环境，也结束本轮并明确暂停原因。任何下游节点必须在下一次对应关键动作中重新 intake 后再路由；本轮输出只报告事实、结果和下一轮入口。

总协调器的输出不是“已写评论”这一动作，而是“当前上下文、当前责任节点、节点技能执行结果、下一步路由”。评论只是跨轮次保存状态的载体。

### 1. Intake 和上下文理解

在读取门禁命中的关键动作开始且 Issue 标识已由用户提供或确认时，调用 `dc-issue-intake` 获取 Issue、完整评论和必要附件，并确认 intake 覆盖状态为 `FULL`。不要用更早动作的 Issue context 或评论缓存替代当前关键动作 intake；不要再运行独立的时间线归约脚本，也不要把结构化事件的字段合并结果当成自动状态机。模型直接阅读本次返回的完整原始时间线，区分结构化交付事实、普通讨论、用户最新指令、已确认决策、冲突和未决问题，再形成当前上下文理解。普通交互沿用当前动作已获取的上下文，除非用户显式刷新或进入新的关键动作。
