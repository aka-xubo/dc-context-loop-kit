...
REQ 和 SPEC 事件直接保存发布时的完整快照；模型结合完整时间线判断当前适用的定义。快照内部的 `REQ-*`、`SCN-*`、`CHK-*`、`AST-*` 只用于当前内容的引用。IMPLEMENTATION 和 ACCEPTANCE 是独立事实，每次完成动作都创建新的 `IMP-*` 或 `ACC-*`。
...
讨论只持续到当前动作可以安全执行。内容足够明确时直接继续；低风险且可逆的歧义可以明确假设后继续。收束后把确认结果直接写入对应的 REQ、SPEC、IMPLEMENTATION 或 ACCEPTANCE 事件；若用户要求长期保留某个业务决策，也应落入目标节点字段。未回答的关键问题不能由 Agent 猜测。
...
每条 SPEC 事件都必须一次提供完整 `specification`：`requirement_ref`、`scenarios`、`checks`、`assertions`、`open_questions`。场景至少表达业务结果、`given`、`when`、`then` 和交付面；检查责任表达场景引用、验证类型和验证责任；断言表达 CHK 引用、`outcome_refs`、断言类型和断言描述。人类区展示发布时的完整规格。下一份 SPEC 可以使用不同的对象 ID，模型根据完整历史判断当前适用的定义。
...
REQ/SPEC 每次都发布当时的完整内容。模型结合完整评论时间线判断当前适用的定义；评论时间和结构化事件 ID 只作为引用和排序依据。
...
脚本会生成一份人类可读评论和机器块，并返回 `duplicate`。REQ/SPEC 使用完整快照事件；IMPLEMENTATION/ACCEPTANCE 使用独立完成事实。相同 `event_id` 已存在时跳过重复发布；不同 `IMP-*` 或 `ACC-*` 应保留为新的时间线记录。

REQ/SPEC 评论展示本次完整快照；实现完成评论展示本次摘要、交付面、开发检查和限制，不展示上一次实现对比。摘要由结构化事件生成，不手工维护第二份内容。
