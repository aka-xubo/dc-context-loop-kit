...
Issue 读取由 `dc-context-loop` 统一协调，不由节点技能自行触发。首次加载、正式验收前和用户明确要求刷新时，通过 `dc-issue-intake` 读取 Issue 和全部评论；同一轮的节点调用直接复用当前 Agent 会话中的 Issue context。完整原始时间线交给模型理解；模型结合评论内容、结构化事件、本地代码和用户最新指令，总结当前上下文并判断下一步。时间和评论 ID 只作为阅读历史时的排序依据。

```text
按门禁读取原始评论 → 模型总结当前上下文 → 判断当前节点 → 调用节点协调技能并复用当前上下文 → 发布事件后按 API 结果结束发布动作 → 路由下一节点
```

需求、实现和验收都可以持续多轮进行。每次需要跨轮恢复时由总协调器按读取门禁获得完整历史；同一轮内不因节点切换重复读取，由模型结合当前会话上下文和用户最新指令选择最小可推进节点。

## 角色定位与边界

使用 `dc-issue-intake` 获取实时 Issue Context Package；不要使用陈旧评论缓存。只有用户提供或确认唯一 Issue 标识后才能执行 intake；无法唯一确定仓库或 Issue 对应的本地上下文时，先询问，不猜测。当前会话中已完成的 intake 结果由节点复用，不创建额外的上下文版本副本。

本技能是 Issue 级总协调器，不只是评论读取器或评论渲染器。它负责按读取门禁让模型从完整时间线恢复当前理解，判断当前应进入哪个节点，调用对应的节点协调技能，并在同一轮把当前会话上下文交给节点复用；它持续推进直到遇到人工决策、真实阻塞或明确结束。节点完成后不因事件发布成功而自动重新 intake。
...
`dc-context-loop` 的核心职责是语义理解和路由，不是把评论字段机械合并成状态机。每次触发完整 intake 后，模型必须以以下材料为输入；未触发 intake 的同轮节点继续使用已在当前会话中的同一份材料：
...
1. 首次处理 Issue、进入正式 ACCEPTANCE 前，或用户明确要求重新读取时，先通过 `dc-issue-intake` 完整读取 Issue 评论，并让模型总结当前上下文。
...
在读取门禁触发且 Issue 标识已由用户提供或确认时，调用 `dc-issue-intake` 获取 Issue、完整评论和必要附件，并确认 intake 覆盖状态为 `FULL`。普通节点交互不重复调用；不要再运行独立的时间线归约脚本，也不要把结构化事件的字段合并结果当成自动状态机。模型直接阅读当前会话中已有的完整原始时间线，区分结构化交付事实、普通讨论、用户最新指令、已确认决策、冲突和未决问题，再形成当前上下文理解。
