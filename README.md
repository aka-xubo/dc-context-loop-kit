...
第一次处理、需求或规格设计、实现方案设计、正式验收，以及你明确要求刷新上下文时，从 `dc-context-loop` 开始：

```text
处理 Issue <URL 或 Issue Key>，先读取完整上下文并判断当前应该进入哪个节点。
```

它会在关键动作前读取完整评论和必要附件，结合本地材料与当前消息判断当前节点。不要直接跳到实现或验收技能。
...
1. 关键动作前先读取该 Issue 的完整上下文，再判断当前节点。
...
它统一协调 `dc-issue-intake`，在关键动作前取得新鲜 Context Package；节点技能复用这次上下文，不自行读取 Issue。它不会在同一轮自动从 REQ 跳到 SPEC、从实现跳到验收，也不会因为一个 READY 事件自动开始验收。
...
首次处理、进入 REQ/SPEC/实现方案设计、正式验收或用户明确要求刷新时，必须重新读取完整 Issue。若 intake 为 `INCOMPLETE`，不能声称已经恢复完整历史，也不能据此跳过门禁。
...
| 材料 | 权威职责 | 不应替代什么 |
| --- | --- | --- |
| Issue 评论时间线 | 保存跨轮协作历史、确认、授权和已发布事件的顺序 | 不能只用本地缓存替代关键动作前的完整 intake |
